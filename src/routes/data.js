...
export default function registerDataRoutes(app) {
  ...
  app.post("/api/import", requireAuth, importRateLimit, (req, res) => {
    ...
    if (!validation.ok)
      return res.status(400).json({ error: "invalid_backup", errors: validation.errors });
  ...
  app.post("/api/backups/run", requireAuth, backupRateLimit, async (req, res) => {
    try {
      ...
      res.status(500).json({ error: "backup_failed", detail: String(error.message || error) });
    }
  });
  ...
  app.get("/api/version", requireAuth, (req, res) => res.json({ version: getDataVersion() }));
  ...
  app.get("/api/export/csv/:type", requireAuth, (req, res) => {
    ...
    if (type === "items") {
      ...
      rows = stmt.allValuations
        .all()
        .map((r) => csvLine([r.id, r.asset_id, r.date, r.value, r.quantity || "", r.note || ""]));
  ...
  app.get("/api/history/charts", requireAuth, (req, res) => {
    ...
    for (const date of dates) {
      ...
      netWorthSeries.push({
        date,
        value: Math.round([...latestByAsset.values()].reduce((s, x) => s + x, 0) * 100) / 100,
      });
    }
