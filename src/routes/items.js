...
import { normalizeItemInput, currentPlanId, getGoals } from "../store.js";
import { textValue, positiveNumber, isoDateValue, oneOf, todayISO } from "../sanitize.js";
...
export default function registerItemRoutes(app) {
  ...
  app.put("/api/items/:id", requireAuth, (req, res) => {
    ...
    if (goal && Number(goal.target_amount) !== payload.cost) {
      stmt.upsertGoal.run({
        ...
        status: (Number(goal.saved_amount) || 0) >= payload.cost ? "complete" : "active",
      });
    }
  ...
  app.post("/api/items/:id/savings", requireAuth, (req, res) => {
    ...
    const goalPayload = {
      ...
      deadline: isoDateValue(req.body?.deadline) || existingGoal?.deadline || item.deadline,
