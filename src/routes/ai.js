...
export default function registerAiRoutes(app) {
  ...
  app.post("/api/ai/chat/stream", requireAuth, aiRateLimit, async (req, res) => {
    ...
    try {
      ...
      if (!aiEnabled()) {
        ...
        return res.end("AI-ассистент не настроен. Добавьте AI_PROVIDER и AI_API_KEY в окружении.");
      }
      res.setHeader("Content-Type", "text/plain; charset=utf-8");
      res.setHeader("X-Accel-Buffering", "no");
      res.flushHeaders?.();
      await askAssistantStream(messages, buildAIContext("balanced"), (delta) => res.write(delta));
      res.end();
    } catch (e) {
  ...
  app.post("/api/ai/month-review", requireAuth, aiRateLimit, async (req, res) => {
    try {
      ...
      const row = planId ? stmt.planById.get(planId) : stmt.closedPlans.all()[0];
      const plan = rowToPlan(row);
      if (!plan || plan.status !== "closed")
        return res.status(404).json({ error: "closed_plan_not_found" });
      const out = await askAssistantText(monthReviewPrompt(plan), buildAIContext("balanced"));
      res.json(out);
    } catch (e) {
      res.status(500).json({ error: "ai_failed", detail: String(e.message || e) });
    }
