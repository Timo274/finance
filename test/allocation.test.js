...
describe('allocate', () => {
  ...
  it('allocates items that fit in budget', () => {
    const items = [
      makeItem({ id: 1, title: 'A', cost: 3000, type: 'must', priority: 5, layer: 'quality' }),
      makeItem({ id: 2, title: 'B', cost: 2000, type: 'should', priority: 3, layer: 'quality' }),
    ];
  ...
  it('respects type priority (must before should before nice)', () => {
    const items = [
      makeItem({ id: 1, title: 'Nice', cost: 2000, type: 'nice', priority: 1, layer: 'quality' }),
      makeItem({ id: 2, title: 'Should', cost: 3000, type: 'should', priority: 1, layer: 'quality' }),
      makeItem({ id: 3, title: 'Must', cost: 4000, type: 'must', priority: 1, layer: 'quality' }),
    ];
    const r = allocate(basePlan, items);
...
describe('scoreVerdict', () => {
  ...
  it('returns keep for high scores', () => {
    const item = makeItem({
      scoreType: 'quick',
      scores: { retained_utility: 5, trajectory_alignment: 5, emotional_trigger: 1, capital_velocity: 5, predicted_30d: 5 },
    });
  ...
  it('returns drop for low scores', () => {
    const item = makeItem({
      scoreType: 'quick',
      scores: { retained_utility: 1, trajectory_alignment: 1, emotional_trigger: 5, capital_velocity: 1, predicted_30d: 1 },
    });
    const v = scoreVerdict(item);
...
describe('tradeoff', () => {
  it('calculates remaining if item added', () => {
    const items = [makeItem({ id: 1, title: 'Existing', cost: 5000, type: 'should', layer: 'quality' })];
  ...
  it('calculates freed amount if item removed', () => {
    const items = [makeItem({ id: 1, title: 'Existing', cost: 5000, type: 'must', priority: 5, layer: 'quality' })];
    const t = tradeoff(1, basePlan, items, {});
    assert.ok(t);
